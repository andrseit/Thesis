...
import agents.station.communication.StationPDA;
...
public class Station {
    ...
    // integrate agents.station info with agents.station pda (maybe add info into the pda)
    ...
    public void sendSuggestions () {
        for (EVObject ev: incomingRequests) {
            //System.out.println("Station No. " + info.getId() + " sends a suggestion to EV No. " + ev.getId() + "(agents.station id: " + ev.getStationId() + ")");
    ...
    public void printWhoCharges () {
        ...
        for (int charges : whoCharges) {
            System.out.println(charges);
        }
    }

    /**
     * A agents.station can receive requests from EVs and save them into a list
    ...
    private String printAnyMap (String name, int[][] scheduleMap, int[] chargers, ArrayList<EVObject> evs) {
        ...
        if (name.equals("Temporary")){
            for (int s = 0; s < chargers.length; s++) {
                str.append(schedule.getRemainingChargers()[s]).append("  ");
            }
        ...
        if (!evs.isEmpty()) {
            for (int ev = 0; ev < scheduleMap.length; ev++) {
                for (int slot = 0; slot < scheduleMap[ev].length; slot++) {
                    str.append(scheduleMap[ev][slot]).append("  ");
                }
                str.append(" : EV No ").append(evs.get(ev).getId()).append(" (").append(evs.get(ev).getStationId()).append(")\n");
            }
        } else {
